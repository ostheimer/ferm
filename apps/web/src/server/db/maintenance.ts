...
export interface DatabaseMaintenanceResult {
  ...
  counts: {
    ...
    reviereinrichtungen: number;
    sitzungen: number;
    protokollVersionen: number;
    beschluesse: number;
    dokumente: number;
  };
}
...
export async function runDatabaseMaintenance(): Promise<DatabaseMaintenanceResult> {
  ...
  try {
    ...
    const [databaseName, appliedMigrations, counts] = await Promise.all([
      readDatabaseName(db),
...
async function readTableCounts(db: ReturnType<typeof createDbFromPool>) {
  const queries = {
    users: 'select count(*)::int as count from "users"',
    reviere: 'select count(*)::int as count from "reviere"',
    memberships: 'select count(*)::int as count from "memberships"',
    notifications: 'select count(*)::int as count from "notifications"',
    reviereinrichtungen: 'select count(*)::int as count from "reviereinrichtungen"',
    sitzungen: 'select count(*)::int as count from "sitzungen"',
    protokollVersionen: 'select count(*)::int as count from "protokoll_versionen"',
    beschluesse: 'select count(*)::int as count from "beschluesse"',
    dokumente: 'select count(*)::int as count from "dokumente"'
  } as const;

  const rows = await Promise.all(
    Object.entries(queries).map(async ([key, statement]) => {
  ...
  return Object.fromEntries(rows) as DatabaseMaintenanceResult["counts"];
}
