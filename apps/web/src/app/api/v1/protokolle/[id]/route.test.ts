...
const { mockGetRequestContext, mockGetProtokollDetail } = vi.hoisted(() => ({
  mockGetRequestContext: vi.fn(),
  mockGetProtokollDetail: vi.fn()
}));
...
vi.mock("../../../../../server/modules/protokolle/queries", () => ({
  getProtokollDetail: mockGetProtokollDetail
}));

import { GET } from "./route";

describe("GET /api/v1/protokolle/:id", () => {
  beforeEach(() => {
    mockGetRequestContext.mockReset();
    mockGetProtokollDetail.mockReset();
  ...
  it("returns a published protocol detail", async () => {
    mockGetProtokollDetail.mockResolvedValue({
      id: "sitzung-2",
      revierId: "revier-attersee",
    ...
    const response = await GET(
      new Request("http://localhost/api/v1/protokolle/sitzung-2"),
      {
        params: Promise.resolve({ id: "sitzung-2" })
      }
    );
  ...
  it("returns 404 when the protocol is not published", async () => {
    mockGetProtokollDetail.mockResolvedValue(undefined);

    const response = await GET(
      new Request("http://localhost/api/v1/protokolle/missing"),
      {
