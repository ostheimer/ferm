...
- `Sprint 4` ist weitgehend abgeschlossen (Reviermeldungen, Aufgaben, Kontaktlisten, echte Karten, Benachrichtigungen vollständig implementiert): Filter, Sort, Detail-Pages, CSV-Export, Status-Mutationen, Aufgaben-Detail, Reviermeldung→Aufgabe-Konvertierung, Benachrichtigungen, Kontaktlisten und echte Karten (Google Maps Web via `@vis.gl/react-google-maps` + Mobile via `react-native-maps`) sind fertiggestellt (PRs #88–#109).
